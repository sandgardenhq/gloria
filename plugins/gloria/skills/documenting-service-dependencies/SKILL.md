...
| --------------- | ------------------- | ------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------- |
| `"http-bearer"` | **Bearer token**    | same as `http`, plus `token` (a secret — see below)                                   | **Default whenever it fits.** The service is reachable with an `Authorization: Bearer …` token — the app holds a Bearer token for it (token-gated API, private resource). Prefer this over `http` whenever a Bearer-authenticated endpoint exists, **even if a public liveness endpoint also exists**. |
...
**The selection rule (authenticated wins):** First ask *does an authenticated check exist for this service* — does the app hold a credential for it, or does it offer an account / whoami / token-gated endpoint? If **yes**, use an authenticated probe — `"http-bearer"` against that endpoint (token declared as a secret), or, when the auth style can't be expressed as an embedded probe, omit `healthCheck` and use a connector (see below). Use `"http"` **only** when the answer is *no* — no credential, no authenticated endpoint. Concretely: if hitting the endpoint the app relies on *without* a credential would not prove your access works, do **not** use `"http"`. _Example: a **private** GitHub repo. `https://api.github.com/zen` is public, so an `http` probe there only proves GitHub-the-service is up — it says nothing about whether your token can still reach the repo. Probe `https://api.github.com/repos/OWNER/REPO` (or `/user`) with `type: "http-bearer"` so a revoked/expired token surfaces as a real failure._
...
**Whenever an authenticated connector exists for the service, the connector wins:** **never** emit an unauthenticated `http` probe for that service just because it also has a public endpoint. **Omit `healthCheck`**, keep the authenticated probe in the Markdown health-check doc, and tell the user to attach the connector in the web UI. The same holds for any service whose authenticated check the embedded probe can't express even without a named connector — omit `healthCheck` rather than downgrade to a misleading unauthenticated `http` probe. The agent-sync path emits `http`/`http-bearer`; richer connectors are a web-UI upgrade.
...
- ❌ Downgrading to an unauthenticated `type: "http"` probe for a service that has a **dedicated authenticated connector** (GitHub, GitLab, Bitbucket, OpenAI, Anthropic, Notion, Linear, Slack, Clerk, Atlassian/Jira, AWS). If a connector exists, the connector wins — omit `healthCheck` and steer the user to it; never emit an unauthenticated probe just because the service has a public endpoint.
