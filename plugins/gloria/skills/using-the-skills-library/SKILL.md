...
|------|-----|----------|---------|
| `search_skills` | Find an existing skill | `query` (optional — omit to list all) | Summaries: slug, name, description, latest version |
...
1. `search_skills` with the task keywords. The query is a **substring match on name and description only**, so it misses differently-named skills — run 2–3 synonym queries ("API key rotation", "credential rotation", "secret rotation"). If still unsure, call it with **no query** to list the whole library.
...
- Searching once with one phrase and concluding "none exists" — the search is a plain substring match; try synonyms or list all.
